...
export function Globe({ className, config = GLOBE_CONFIG }: { className?: string; config?: typeof GLOBE_CONFIG }) {
  ...
  const updatePointerInteraction = (value: number | null) => {
    ...
    if (canvasRef.current) {
      (canvasRef.current as HTMLCanvasElement).style.cursor = value !== null ? "grabbing" : "grab";
    }
  };
  ...
  useEffect(() => {
    const onResize = () => {
      if (canvasRef.current) {
        width.current = (canvasRef.current as HTMLCanvasElement).offsetWidth;
      }
    };
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      onRender: (state: unknown) => {
        ...
        if (typeof state === 'object' && state !== null) {
          ...
          s.phi = phi.current + rs.get();
          s.width = width.current * 2;
          s.height = width.current * 2;
        }
      },
    });
